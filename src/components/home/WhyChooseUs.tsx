...
export default function WhyChooseUs() {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setIsVisible(true);
          observer.disconnect(); // Stop observing once it animates in
        }
      },
  ...
  return (
    <section 
      ref={sectionRef} 
      className="w-full bg-[#FFFFFF] px-6 md:px-16 lg:px-[100px] pt-16 pb-20 lg:pt-[60px] lg:pb-[100px] overflow-hidden"
    >
      <div className="w-full max-w-[1720px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-2 lg:gap-y-0 border-gray-200">
          {features.map((feature, index) => {
            // Calculate delay based on index for the staggered cascade effect
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${isVisible ? `animate-base content-animation ${delayClass}` : "opacity-0"}
                `}
              >
                {/* Icon Container */}
